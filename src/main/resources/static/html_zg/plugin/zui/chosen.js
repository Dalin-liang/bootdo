...
(function() {
	...
	SelectParser = (function() {
		...
		SelectParser.prototype.add_group = function(group) {
			...
			this.parsed.push({
						...
						title : group.title,
						search_keys : ($.trim(group.getAttribute('data-keys')
								|| '')).replace(/,/g, ' ')
					});
	...
	AbstractChosen = (function() {
		function AbstractChosen(form_field, options) {
			...
			this.lang = LANGUAGE[this.options.lang
					|| ($.zui.clientLang ? $.zui.clientLang() : 'zh_cn')];
			...
			for (_i = 0, _len = _ref.length; _i < _len; _i++) {
				...
				if (this.include_option_in_results(option)) {
					...
					if (!(option.group && !this.group_search)) {
						...
						if (option.search_match) {
							if (option.search_text_match
									&& option.search_text.length) {
								...
								text = option.search_keys.substr(0, startpos
												+ searchText.length)
										+ '</em>'
										+ option.search_keys.substr(startpos
												+ searchText.length);
								option.search_text += '&nbsp; <small style="opacity: 0.7">'
										+ text.substr(0, startpos)
										+ '<em>'
										+ text.substr(startpos) + '</small>';
							}
	...
	Chosen = (function(_super) {
		...
		Chosen.prototype.set_up_html = function() {
			...
			if (this.options.drop_width) {
				this.dropdown.css('width', this.options.drop_width)
						.addClass('chosen-drop-size-limited');
			}
			this.results_build();
			this.set_tab_index();
		...
		Chosen.prototype.result_do_highlight = function(el, canMiddleHighlight) {
			...
			if (el.length) {
				...
				if (this.middle_highlight
						&& (canMiddleHighlight
								|| this.middle_highlight === 'always'
								|| high_bottom >= visible_bottom || high_top < visible_top)) {
					scrollTop = Math.min(high_top - resultHeight, Math.max(0,
									high_top - (maxHeight - resultHeight) / 2));
		...
		Chosen.prototype.choice_build = function(item) {
			...
			choice = $('<li />', {
				...
				}).html("<span title='" + item.html + "'>" + item.html
					+ "</span>");
			/// ZUI change end
			if (item.disabled) {
		...
		Chosen.prototype.single_set_selected_text = function(text) {
			...
			return this.selected_item.find("span").attr('title', text)
					.text(text);
			/// ZUI change end
		};
