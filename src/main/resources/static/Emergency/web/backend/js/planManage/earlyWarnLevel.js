...
$(document).ready(function(){
	$('#earlyWarnLevelTable').bootstrapTable({
		...
		columns : [{
	            ...
	            },
				{
					...
					formatter: function (value, row, index) {
	                    //获取每页显示的数量
	                    var pageSize=$('#earlyWarnLevelTable').bootstrapTable('getOptions').pageSize;
	                    //获取当前是第几页
	                    var pageNumber=$('#earlyWarnLevelTable').bootstrapTable('getOptions').pageNumber;
...
function reloadTable(){
	$('#earlyWarnLevelTable').bootstrapTable('refresh');
}

function edit(id){
	var data = $('#earlyWarnLevelTable').bootstrapTable('getRowByUniqueId',id);//行的数据
	data.dataType = "edit";
    ...
    layer.open({
        ...
        shade: 0.4,
        area: ['800px', '380px'],
        content: 'earlyWarnLevelForm.html'
   }); 
}
