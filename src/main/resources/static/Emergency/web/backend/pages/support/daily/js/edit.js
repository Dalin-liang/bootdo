	...
	layui.use(['form','laydate','upload'], function () {
		...
		if (parent.editData != null && parent.editData != "") {
			...
			if(parentData.dataType =="view"){
				...
				$("#isPersonInCharge").attr('disabled', 'disabled');
			...
			form.val('dailyForm',{
				...
				"isPersonInCharge" : (parentData.is_personincharge == '1') ? true: false
			});
...
function getFileList(dailyId) {
	$.ajax({
		url : '/support/daily/getFileList',
		method : 'post',
		data : {
			dailyId : dailyId
		},
...
function downloadFile(id) {
	window.location.href="/support/daily/downloadFile?id="+id;
