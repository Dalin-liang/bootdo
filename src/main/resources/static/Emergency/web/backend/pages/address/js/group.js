        ...
        columns : [{
        ...
        },
            {
                ...
                formatter: function (value, row, index) {
                    //获取每页显示的数量
                    var pageSize=$('#groupTable').bootstrapTable('getOptions').pageSize;
                    //获取当前是第几页
                    var pageNumber=$('#groupTable').bootstrapTable('getOptions').pageNumber;
...
function edit(id){
    var data = $('#groupTable').bootstrapTable('getRowByUniqueId',id);//行的数据
    data.dataType = "edit";
...
function deleteData(id){
    ...
    layer.confirm('确定要删除该行数据吗？', function(index){
        $.ajax({
            url: prefix + '/removeGroupAndPerson',
            method:'post',
            data:{
                "id": id
            },
