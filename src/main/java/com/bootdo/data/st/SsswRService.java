...
/**
 * 实时水位表:用于存储测站上报的实时水位数据，水位数据为瞬时水位，同一个测站下的指定传感器在某一个时刻只能有一条实时水位记录
 * 
 * @author wux
 * @email oking@163.com
 ...
 */
@Component
public class SsswRService {
	@Autowired
	private ReceiveinfoService receiveinfoService;
	
	public void getData()  {
		System.out.println("===execute==ssswR==");
		String baseUrl = (String) GenUtils.getConfig().getProperty("big_data_center_baseUrl");
		String url = baseUrl+"api/st/ssswR/getByBeginTime";
