...
/**
 * 根据开始时间结束时间查询汽车设备里程统计数据
 *  @author qiaodk
 */
@Component
public class CarDevMileStatInfoJob implements Job {

    @Autowired
    private ReceiveInfoMapper receiveInfoMapper;

    private static final Logger logger = LoggerFactory.getLogger(CarDevMileStatInfoJob.class);

    @Override
    public void execute(JobExecutionContext context) throws JobExecutionException {
        System.out.println("===execute==CarDevMileStatInfoJob==");
        String baseUrl = (String) GenUtils.getConfig().getProperty("big_data_center_baseUrl");
        String url = baseUrl+"cardata/carServiceMonitor/getDevMileStatInfo";
