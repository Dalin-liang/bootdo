...
/**
 * 根据开始时间结束时间查询汽车设备逆地理位置数据
 *  @author qiaodk
 */
@Component
public class CarDevInverseAddressJob implements Job {

    @Autowired
    private ReceiveInfoMapper receiveInfoMapper;

    private static final Logger logger = LoggerFactory.getLogger(CarDevInverseAddressJob.class);

    @Override
    public void execute(JobExecutionContext context) throws JobExecutionException {
        System.out.println("===execute==CarDevInverseAddressJob==");
        String baseUrl = (String) GenUtils.getConfig().getProperty("big_data_center_baseUrl");
        String url = baseUrl+"cardata/carServiceMonitor/getDevInverseAddress";
