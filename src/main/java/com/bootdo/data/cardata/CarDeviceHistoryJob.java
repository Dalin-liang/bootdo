...
/**
 * 根据开始时间结束时间查询汽车设备历史轨迹数据
 * @author qiaodk
 */
@Component
public class CarDeviceHistoryJob implements Job {
    @Autowired
    private ReceiveInfoMapper receiveInfoMapper;

    private static final Logger logger = LoggerFactory.getLogger(CarDeviceHistoryJob.class);

    @Override
    public void execute(JobExecutionContext context) throws JobExecutionException {
        System.out.println("===execute==CarDeviceHistoryJob==");
        String baseUrl = (String) GenUtils.getConfig().getProperty("big_data_center_baseUrl");
        String url = baseUrl+"cardata/carServiceMonitor/getDevHistory";
