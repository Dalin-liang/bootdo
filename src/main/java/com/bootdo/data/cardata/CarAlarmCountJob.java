...
/**
 * 根据开始时间结束时间查询汽车报警总数数据
 * @author qiaodk
 */
@Component
public class CarAlarmCountJob implements Job {

    @Autowired
    private ReceiveInfoMapper receiveInfoMapper;

    private static final Logger logger = LoggerFactory.getLogger(CarAlarmCountJob.class);

    @Override
    public void execute(JobExecutionContext context) throws JobExecutionException {
        System.out.println("===execute==CarAlarmCountJob==");
        String baseUrl = (String) GenUtils.getConfig().getProperty("big_data_center_baseUrl");
        String url = baseUrl+"cardata/carServiceMonitor/getDevAlarmCount";
    ...
    public static void main(String[] args){
        String aa = "1568822400000";
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        aa=sdf.format(new Date(Long.parseLong(aa)));
