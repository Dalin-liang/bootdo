...
import java.util.*;
/**
 * 根据开始时间结束时间查询汽车设备信息数据
 * @author qiaodk
 */
@Component
public class CarDeviceInfoJob implements Job {
    @Autowired
    private ReceiveInfoMapper receiveInfoMapper;

    private static final Logger logger = LoggerFactory.getLogger(CarDeviceInfoJob.class);

    @Override
    public void execute(JobExecutionContext context) throws JobExecutionException {
        System.out.println("===execute==CarDeviceInfoJob==");
        String baseUrl = (String) GenUtils.getConfig().getProperty("big_data_center_baseUrl");
        String url = baseUrl+"cardata/carServiceMonitor/getDevInfos";
