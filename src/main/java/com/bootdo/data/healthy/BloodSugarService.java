...
/**
 * 定时器：根据开始时间和结束时间获取血糖数据
 * @author wux
 *
 */
@Component
public class BloodSugarService implements Job {
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		System.out.println("===execute==BloodSugarService==");
		String baseUrl = (String) GenUtils.getConfig().getProperty("big_data_center_baseUrl");
		String url = baseUrl+"api/healthy/healthyUser/getBloodSugar";
		...
		if(StringUtils.isNotEmpty(eventdesc)){
	        ...
	        for(String desc:descs){
	        	...
	        	if(StringUtils.isNotEmpty(data[1].toString())){
	        		if("bloodsugar".equals(data[0])){
		        		bloodsugar = Double.valueOf(data[1].toString());
		        	}else if("type".equals(data[0])){
	        ...
	        if(bloodsugar !=0.0){
	            ...
	            case 2://<7.0
	            	...
	            	break;
	            case 3://6.7-9.4
	            	if(bloodsugar <6.7 ||bloodsugar >9.4){
	            		return true;
	            	}
	            	break;
	        }
