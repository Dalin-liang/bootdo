...
/**
 * 定时器：根据开始时间和结束时间获取血压数据
 * @author wux
 *
 */
@Component
public class BloodPressureService implements Job {
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		System.out.println("===execute==BloodPressureService==");
		String baseUrl = (String) GenUtils.getConfig().getProperty("big_data_center_baseUrl");
		String url = baseUrl+"api/healthy/healthyUser/getBloodPressure";
		...
		if(StringUtils.isNotEmpty(eventdesc)){
	        ...
	        for(String desc:descs){
	        	...
	        	if(StringUtils.isNotEmpty(data[1].toString())){
		        	...
		        	switch(key) {
			            case "systolic"://90~139
			                if(value <90 ||value >139){
			                	return true;
			                }
			            	break;
			            case "diastolic"://60~89
