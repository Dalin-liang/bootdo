...
/**
 * 定时器：根据开始时间和结束时间获取身高体重数据
 * @author wux
 *
 */
@Component
public class HeightWeightService implements Job {
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		System.out.println("===execute==HeightWeightService==");
		String baseUrl = (String) GenUtils.getConfig().getProperty("big_data_center_baseUrl");
		String url = baseUrl+"api/healthy/healthyUser/getHeightWeight";
		...
		if(StringUtils.isNotEmpty(eventdesc)){
	    	...
	    	for(String desc:descs){
	        	...
	        	if(StringUtils.isNotEmpty(data[1].toString())){
	        		if("weight".equals(data[0])){
		        		weight = (Double.valueOf(data[1].toString()))/2;
		        	}else if("height".equals(data[0])){
		        		height = (Integer.valueOf(data[1].toString()))/100;
		        	}
	        	}
	        }
